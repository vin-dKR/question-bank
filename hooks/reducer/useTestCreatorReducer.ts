...
export type TestCreatorAction =
    ...
    | { type: 'SET_QUESTIONS'; questions: QuestionForCreateTestData[] }
    | { type: 'LOAD_QUESTIONS'; questions: QuestionForCreateTestData[] }
    | { type: 'SET_SUBMITTING'; isSubmitting: boolean }
    | { type: 'SET_LOADED_QUESTIONS'; hasLoadedQuestions: boolean };
...
const reducer = (state: TestCreatorState, action: TestCreatorAction): TestCreatorState => {
    switch (action.type) {
        ...
        case 'APPLY_BULK_MARKS': {
            ...
            const updatedQuestions = state.testData.questions.map((q) => ({
                ...q,
                marks: action.marks,
            }));
        ...
        case 'SET_LOADED_QUESTIONS':
            return { ...state, hasLoadedQuestions: action.hasLoadedQuestions };
        default:
            return state;
    }
};
