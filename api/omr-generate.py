...
from omr.generate import generate  # noqa: E402
...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        try:
            ...
            work_dir = Path(tempfile.mkdtemp(prefix="question-bank-omr-generate-"))
            spec_path = work_dir / "spec.json"
            spec_path.write_text(json.dumps(spec), encoding="utf-8")

            exam_spec = spec_mod.load(spec_path)
            summary = generate(exam_spec, work_dir)
            paths = [Path(summary["pdf"]), *[Path(path) for path in summary["artifacts"]]]
            ...
            write_json(
                ...
                HTTPStatus.OK,
                {
                    "ok": True,
                    "summary": summary,
                    "files": encode_files(paths, work_dir),
                },
            )
