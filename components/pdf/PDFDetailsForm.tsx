...
export default function PDFDetailsForm({ initialData, onSubmit, onCancel, isGenerating }: PDFDetailsFormProps) {
    ...
    const { templates, templatesLoading, saveTemplate: saveTemplateAction, fetchTemplates, removeTemplate, updateTemplate } = usePdfTemplateForm()

    // Keep track of the originally selected template (for diffing)
        ...
        if (validateForm()) {
            ...
            try {
                ...
                if (saveTemplate && !isEditingTemplate) {
                    ...
                    // If editing an existing template and user modified fields, update it
                    if (isEditingTemplate && selectedTemplate?.id) {
                        ...
                        if (hasChanges) {
                            const res = await updateTemplate(selectedTemplate.id, changed);
                            if (!res.success) {
                                alert(`Failed to update template: ${res.error}`);
                                setIsCreatingTemplate(false);
                                return;
                            }
                        }
