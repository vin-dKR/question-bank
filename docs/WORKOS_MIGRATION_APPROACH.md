...
**Onboarding.** The Student card is gone from `constant/on-boarding/user-type.ts`, and `completeOnboarding` rejects `role=student` outright. `Student` and `StudentData` remain for when student logins are switched on; the `/onboarding/student/setup` route was deleted on 25 Aug 2026 — it was unreachable from the UI but still served a form that `completeOnboarding` would refuse to save. Both remaining paths create an org silently — a solo teacher's is named after the school they typed, `type: 'personal'`; a centre's after `centerName`, `type: 'coaching'`.
